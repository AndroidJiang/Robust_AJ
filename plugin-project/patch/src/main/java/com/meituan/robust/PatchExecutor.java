...
public class PatchExecutor extends Thread {
    ...
    protected boolean patch(Context context, Patch patch) {
        ...
        try {
            ...
            robustCallBack.logNotify("patch classLoader failure, patch info:" + "id = " + patch.getName() + ",md5 = " + patch.getMd5()+"msg:"+throwable.getMessage(), "class:PatchExecutor method: DexClassLoader");
        }
        if (null == classLoader) {
...
//                robustCallBack.exceptionNotify(t, "class:PatchExecutor method:patch line:169");
    ...
    private static final String ROBUST_PATCH_CACHE_DIR = "patch_cache_";
    private static final String ROBUST_DIR = "robust";
    /*
     * @param c
     * @return 返回缓存补丁路径，一般是内部存储,补丁目录
     */
    private static File getPatchCacheDirPath(Context c, String key) {
        File parentDir = c.getDir(ROBUST_DIR, Context.MODE_PRIVATE);
        if (!parentDir.exists()) {
            parentDir.mkdir();
        }
        File patchTempDir = new File(parentDir,ROBUST_PATCH_CACHE_DIR + key);
